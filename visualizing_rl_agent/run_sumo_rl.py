...
MODEL_PATH = "best_traffic_agent.pth"
GUI_MODE = True  

# RL Environment Constraints (MUST match env.py for model consistency)
MAX_QUEUE = 20
CORRIDOR_DELAY = 5
...
ARMS_MAPPING = {
    "int_left": {
        0: ["ped1n2il_0"], 1: ["ped1s2il_0"], 
        2: ["hw_ir2il_0", "hw_ir2il_1"],
        3: ["hw_w2il_0", "hw_w2il_1"]
    },
    "int_right": {
        0: ["ped2n2ir_0"], 1: ["ped2s2ir_0"],
...
# Phase Mapping: RL 0 (NS Green) -> SUMO 0, RL 1 (EW Green) -> SUMO 2
SUMO_PHASE_NS_GREEN = 0
SUMO_PHASE_EW_GREEN = 2
...
def run_simulation():
    ...
    agent = DQNAgent(obs_dim=22, n_actions=4)
    ...
    print("Simulation started. Use the Pygame window to see the model's internal view and SUMO for physical view.")
    ...
    try:
        while traci.simulation.getMinExpectedNumber() > 0:
            ...
            for i, intersection_id in enumerate(INTERSECTIONS):
                ...
                sumo_phase = SUMO_PHASE_NS_GREEN if current_phases_rl[i] == 0 else SUMO_PHASE_EW_GREEN
            ...
            if step % 10 == 0:
                print(f"Step {step} | Max Arm Queue: {int(max_q_arm)}/20 | Total (8 arms): {(int(mock_env.queues.sum()))} | Discharged: {total_cars_arrived}")

            # Safety: If SUMO is jammed, don't let it run forever
            if max_q_arm >= 50:
                 print("WARNING: SUMO physics congestion exceeds model capability. Skipping some steps.")
