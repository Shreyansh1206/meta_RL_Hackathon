...
# Insert the PPO directory to the python path to use the exact same agent, config, and visualizer.
sys.path.insert(
    0,
    os.path.abspath(
        os.path.join(os.path.dirname(__file__), "..", "RL-Traffic-Lights-PPO-EV")
    ),
)
...
MODEL_PATH = os.path.join(
    ...
    "..",
    "RL-Traffic-Lights-PPO-EV",
    "best_traffic_agent_ppo_ev.pth",
)
GUI_MODE = True
            ...
            for veh in veh_ids:
                if traci.vehicle.getSpeed(veh) < 0.1:  # Halting threshold
                    if traci.vehicle.getTypeID(veh) == "emergency_car":
                        ...
                        std_halted += 1
        std_queues.append(min(float(std_halted), float(MAX_QUEUE)))
        ev_queues.append(min(float(ev_halted), float(MAX_QUEUE)))
    return std_queues, ev_queues
...
def run_simulation():
    ...
    agent = PPOAgent(obs_dim=obs_dim, n_actions=4)
    try:
    ...
    for intersection_id in INTERSECTIONS:
        ...
        # Lock the phase so SUMO won't auto-advance to yellow/next phase
    ...
    try:
        while traci.simulation.getMinExpectedNumber() > 0:
            ...
            for i, intersection_id in enumerate(INTERSECTIONS):
                std_queues, ev_queues = get_queue_lengths(intersection_id)
                mock_env.queues[i] = np.array(std_queues, dtype=np.int32)
                mock_env.ev_queues[i] = np.array(ev_queues, dtype=np.int32)
                
                obs.extend(std_queues)
                obs.extend(ev_queues)
