...
from config import (
    ...
    EV_SPAWN_RATE_START, EV_SPAWN_RATE_END,
)
...
def evaluate(env, agent, n_episodes=EVAL_EPISODES):
    ...
    for _ in range(n_episodes):
        ...
        while not done:
            ...
            # Use final EV rate for evaluation consistency
            state, reward, terminated, truncated, _ = env.step(action, current_ev_spawn_rate=EV_SPAWN_RATE_END)
...
def train_agent(total_steps=TOTAL_TIMESTEPS, checkpoint=CHECKPOINT_PATH):
    ...
    current_ev_rate = EV_SPAWN_RATE_START
    state, _ = env.reset()

    while step_count < total_steps:
        # Linear decay: reaches EV_SPAWN_RATE_END at total_steps / 2
        frac = min(1.0, step_count / (total_steps / 2.0))
        current_ev_rate = EV_SPAWN_RATE_START - frac * (EV_SPAWN_RATE_START - EV_SPAWN_RATE_END)

        # ── collect rollout ───────────────────────────────────────────────
        for _ in range(ROLLOUT_LENGTH):
            action, log_prob, value = agent.act(state)
            next_state, reward, terminated, truncated, info = env.step(action, current_ev_spawn_rate=current_ev_rate)
            done = terminated or truncated
